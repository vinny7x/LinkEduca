...
<div className="grid grid-cols-1 md:grid-cols-2 gap-4 ml-[10%] mr-[10%] mt-2">
  {data && data.map((news: news, i: number) => (
    <div 
      key={i} 
    ...
    >
      <img 
        ...
        alt={news.title} 
      />
      <h1 className="text-lg font-bold mt-3 text-center">{news.title}</h1>
      <h3 className="text-[var(--color-text)] text-sm opacity-90 mt-1 text-center flex-grow">{news.desc}</h3>
      <a 
